...
class GazzettaXmlParser(XmlParser):
    ...
    def parse(self):
        ...
        for channel in rss.find_all("channel"):
            ...
            result = GazzettaChannel(title, description, link)

            for item in channel.find_all_next("item"):
                result.add_item(GazzettaItem(
...
class GazzettaChannel(Channel):
    def __init__(self, title, description, link):
        super().__init__(title, description, link)


class GazzettaItem(Item):
